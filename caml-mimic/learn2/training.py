...
import interpret
import persistence
import learn2.models as models
import learn2.tools as tools
...
def test(model, Y, epoch, data_path, fold, gpu, version, code_inds, dicts, samples, model_dir, testing):
    ...
    metrics['loss_%s' % fold] = torch.mean(torch.stack(losses))
    return metrics
...
if __name__ == "__main__":
    ...
    parser.add_argument("model", type=str, choices=["cnn_vanilla", "rnn", "conv_attn", "multi_conv_attn", "han", "logreg", "saved"], help="model")
